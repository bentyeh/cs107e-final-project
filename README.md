...
Eric - I wrote the audio library, the sensor read functions, and constructed the physical mockup (wiring, breadboarding, soldering, etc.). To write the audio library, I modified the PWM lecture code by adding wave mixing functions (in audio.c). To do this, I added functionality to the minimath library. For the sensors I wrote a function/library on top of the cs107e SPI library to record the voltage of the piezo sensors at the time of reading. Finally, I constructed the final mock-up of our project as well as the various different test setups. I did not use any code that I did not write besides code found in the cs107e.github.io repository.
